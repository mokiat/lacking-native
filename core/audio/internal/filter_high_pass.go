...
// HighPassFilter is a stereo Processor that applies a second-order Butterworth
// high-pass filter to each channel. The cutoff frequency is safe to set from
// any goroutine.
type HighPassFilter struct {
	...
	filterL *HighPassSampleFilter
	filterR *HighPassSampleFilter
}

var _ audio.FrequencyFilter = (*HighPassFilter)(nil)
var _ Processor = (*HighPassFilter)(nil)

// NewHighPassFilter creates a HighPassFilter with a 350 Hz initial cutoff for
...
func NewHighPassFilter(sampleRate int) *HighPassFilter {
	return &HighPassFilter{
		...
		filterL: NewHighPassSampleFilter(sampleRate),
		filterR: NewHighPassSampleFilter(sampleRate),
	}
}
...
// Process applies the high-pass filter to inputFrames and returns the result.
func (f *HighPassFilter) Process(ctx ProcessContext, inputFrames []audio.Frame) []audio.Frame {
	f.mu.Lock()
